...
## Entries are bound to specific hulls by hull_id, exactly as the pre-battle
## screen does — the panel maps entries to hulls by that id.
func _make_entries() -> Array:
	...
	return BattlePlanner.assign_hull_ids(entries, RoguelikeRun.sortieable_hulls())
...
func test_roster_mode_offers_a_hull_per_crew_group():
	...
	for option in panel._ship_options:
		if option.kind == DoctrinePanel.KIND_HULL:
			hull_count += 1
	assert_eq(hull_count, RoguelikeRun.fleet_hulls.size(),
		"Roster mode should offer every hull, with no battle plan")
