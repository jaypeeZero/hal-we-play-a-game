...
func test_damaged_power_core_reduces_weapon_damage():
	...
	if result.fire_commands.size() > 0:
		...
		assert_le(actual_damage, base_damage, "Damaged power core should reduce weapon damage")
...
func test_damaged_control_reduces_accuracy():
	...
	if result.fire_commands.size() > 0:
		...
		assert_le(actual_accuracy, base_accuracy, "Damaged control should reduce accuracy")
...
func test_get_fireable_weapons_returns_ready_weapons_only():
	...
	assert_le(fireable.size(), ship.weapons.size(), "Should return subset of weapons")
...
func test_calculate_hit_probability_returns_valid_range():
	...
	assert_ge(probability, 0.0, "Hit probability should be >= 0")
	assert_le(probability, 1.0, "Hit probability should be <= 1")
