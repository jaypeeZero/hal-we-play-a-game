...
func update_state(entity_id: String, state: EntityState) -> void:
	...
	EntityOverlays2D.update_wing_circle(visual.root, state.wing_color)

	# Green pulse while an engineer's repair is landing
	EntityOverlays2D.update_repair_indicator(visual.root, state.has_flag("repairing"))

	# Update debug visuals
	EntityOverlays2D.update_pilot_direction_line(visual.root, state.debug_pilot_direction)
	EntityOverlays2D.update_leader_number(visual.root, state.debug_leader_number)
