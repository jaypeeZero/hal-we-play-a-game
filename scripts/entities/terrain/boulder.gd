...
# Boulder is an obstacle that blocks movement for both creatures and players
...
func _setup_blocking_collision() -> void:
	...
	static_body.name = "BoulderBlocker"
...
func _on_object_entered(object: BattlefieldObject) -> void:
	# Boulder blocks movement via StaticBody2D - no action needed
	pass
