...
## Lifecycle: standalone scene reached from the title menu — edits the global
## crew roster template (not a run's hired crew). Back returns to the main menu.
...
func _ready() -> void:
	"""Build the Crew Manager (global roster editor) screen."""
