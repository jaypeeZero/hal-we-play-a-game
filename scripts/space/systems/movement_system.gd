...
## Distance-aware: At range uses main thrust to approach, up close uses lateral to slide into position
static func calculate_flank_behind(ship_data: Dictionary, target: Dictionary, nearby_ships: Array, obstacles: Array) -> Dictionary:
	...
	var distance_to_behind = to_behind.length()

	# Distance-aware behavior
	if distance_to_target > LATERAL_THRUST_RANGE:
		# FAR: Use main thrust to approach behind position
		var desired_heading = direction_to_heading(to_behind)

		# DART AND DASH: Sharp turns to get behind enemy
		var needs_course_correction = check_needs_braking(ship_data, desired_heading)
		if needs_course_correction:
			return create_braking_control(ship_data, desired_heading, distance_to_behind)

		# Flanking uses moderate throttle - positioning, not charging
		var throttle = calculate_intuitive_throttle(ship_data, distance_to_behind, "flanking")
		...
		# CLOSE: Face target, use lateral thrust to slide into behind position
		var desired_heading = direction_to_heading(to_target)

		# Calculate lateral thrust to slide toward behind position
		...
		if current_velocity.length() > max_flank_speed:
			should_brake = true
			throttle = 0.0
