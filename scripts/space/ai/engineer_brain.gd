...
## GOAP-style planner for engineers.
## Filters actions by precondition → sorts by cost → plan lock → cheapest wins.
## Plan lock rarely engages at second-scale cadences but keeps brains uniform.
...
static func decide(ws: EngineerWorldState, game_time: float) -> Dictionary:
...
static func _get_actions() -> Array:
	return [
		RepairInternalAction.new(),
		RepairArmorAction.new(),
	]
