class_name GunnerBrain
extends RefCounted

## GOAP-style planner for gunners.
## Filters actions by precondition → sorts by cost → plan lock → cheapest wins.
## Plan lock prevents fire-mode flip-flopping between knowledge queries.
...
static func decide(ws: GunnerWorldState, game_time: float) -> Dictionary:
...
static func _get_actions() -> Array:
	return [
		HoldFireAction.new(),
		SuppressiveFireAction.new(),
		PrecisionShotAction.new(),
		StandardFireAction.new(),
	]
