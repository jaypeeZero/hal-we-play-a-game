class_name SquadronLeaderBrain
extends RefCounted

## GOAP-style planner for squadron leaders.

const PLAN_LOCK_DURATION          = 1.2
const PRIORITY_OVERRIDE_THRESHOLD = 0.5


static func decide(ws: SquadronLeaderWorldState, game_time: float) -> Dictionary:
...
static func _get_actions() -> Array:
	return [
		CallMutualSupportAction.new(),
		ReformFormationAction.new(),
		ScreenWithdrawalAction.new(),
		CoordinateAttackRunAction.new(),
		AssignTargetsAction.new(),
	]
